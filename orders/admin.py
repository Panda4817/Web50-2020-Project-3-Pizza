...
class OrderToFoodAdmin(admin.ModelAdmin):
    model = OrderToFood
    list_display = ['pk', 'order', 'food_item', 'get_toppings', 'get_extra', 'quantity']
...
class OrderAdmin(admin.ModelAdmin):
    model = Order
    list_display = ['pk', 'user', 'total', 'status', 'delivery_type', 'delivery_intructions']

class AddressAdmin(admin.ModelAdmin):
    model = Address
    list_display = ['pk', 'user', 'street_address_1', 'street_address_2', 'city', 'post_code', 'email_confirmed', 'tel']
